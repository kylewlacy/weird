...
export class Lexer {
    ...
    bytePos = 0; // byte position for spans
    line = 1;
    col = 1;
    constructor(source) {
        this.source = source;
    ...
    readQuotedString(start, hadWhitespace, hadNewline) {
    ...
    readRawString(start, hadWhitespace, hadNewline) {
        this.advance(); // r
        let hashes = 0;
        while (this.peek() === "#") {
            this.advance();
    ...
    readHeredoc(start, hadWhitespace, hadNewline) {
        this.advance(); // <
        this.advance(); // <
    ...
    dedentHeredoc(content, indentLen) {
    ...
    readBareScalar(start, hadWhitespace, hadNewline) {
        let text = "";
        while (this.pos < this.source.length) {
            const ch = this.peek();
